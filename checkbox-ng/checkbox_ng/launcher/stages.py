...
class MainLoopStage(CheckboxUiStage):
    ...
    def _run_single_job_with_ui_loop(self, job, ui):
        ...
        while True:
            if job.plugin in (
                ...
                if not self.is_interactive and job.plugin in (
                    "user-interact",
                    "user-interact-verify",
                    "manual",
                ):
                ...
                if self.is_interactive and job.plugin in (
                    "user-interact",
                    "user-interact-verify",
                    "manual",
                ):
            ...
            if (
                self.is_interactive
                and result_builder.outcome == IJobResult.OUTCOME_UNDECIDED
            ):
                try:
                    if comments != "":
                        result_builder.comments = comments
    ...
    def _interaction_callback(
        self, job, job_state, result_builder, prompt=None, allowed_outcome=None
    ):
    ...
    def _run_jobs(self, jobs_to_run):
        ...
        already_done = list(filter(lambda x: job_done(x[2]), job_id_def_states))
        still_todo = list(filter(lambda x: not job_done(x[2]), job_id_def_states))
    ...
    def _run_setup_jobs(self, jobs_to_run):
        ...
        return list(
            filter(
                lambda x: x[1] in [IJobResult.OUTCOME_FAIL, IJobResult.OUTCOME_CRASH],
                job_id_outcome,
            )
        )
    ...
    def _generate_tp_infos(self, tp_list):
        ...
        for tp_id in tp_list:
            tp_info = {"id": tp_id, "name": self.sa.get_test_plan(tp_id).name}
...
class ReportsStage(CheckboxUiStage):
    ...
    def _create_transport(self, transport):
        ...
        if tr_type not in self._available_transports:
            _logger.error(
                _("Unrecognized type '%s' of transport '%s'"),
                tr_type,
                transport,
            )
            raise SystemExit(1)
    ...
    def _export_results(self):
        ...
        for name, params in sorted(
            ...
            # development
            if (
                params.get("transport") == "certification"
                and self.sa.sideloaded_providers
            ):
                _logger.warning(
                    _("Using side-loaded providers disabled " "the %s report"),
                    name,
            ...
            if self.is_interactive and not params.get("forced", False):
                ...
                cmd = self._pick_action_cmd(
                    [Action("y", _("yes"), "y"), Action("n", _("no"), "n")],
                    message,
                )
            else:
                cmd = "y"
            ...
            while not done_sending:
                try:
                    ...
                    if url:
                        path = self._get_submission_file_path(".submission.log")
