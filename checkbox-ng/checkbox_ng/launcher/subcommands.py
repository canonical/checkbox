...
class Launcher(MainLoopStage, ReportsStage):
    ...
    def is_interactive(self):
        ...
        """
        return (
            self.launcher.ui_type == "interactive"
    ...
    def _maybe_auto_resume_session(self, resume_candidates):
        ...
        pdb.set_trace()
        sessions = [session.id for session in resume_candidates]
        if sessions:
            selected_session_id = ResumeSessionBrowser(sessions).run()
            raise SystemExit(selected_session_id)
        print("no sessions")
        """
    ...
    def _maybe_manually_resume_session(self, resume_candidates):
        ...
        chosen_session, chosen_action, comment = ResumeMenu(entries).run()
        if chosen_session:
            self._resume_session(chosen_session)
            return True
        return False
