...
from checkbox_ng.support.parsers.sysfs_usb import ishex
from checkbox_ng.support.parsers.sysfs_usb import UsbIds
...
class TestUsbIds(TestCase):
    ...
    def test_full_protocol(self, m_isfile):
        ...
        with patch("builtins.open", mopen):
            ids = UsbIds()
            self.assertEqual(
                ids.decode_protocol(0x42, 0x06, 0x01), "Explosives:Bomb:Boom"
            )
