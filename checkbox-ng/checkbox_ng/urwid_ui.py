    ...
    def _job_detail_view(self, node):
        ...
        for test_info in test_info_list:
            if test_info["id"] == node.get_key():
                ...
                break
        contents = [
            urwid.Text(("focus", " Job Details "), "center"),
            urwid.Divider(),
        ]
        ...
        if self.get_depth() == 1:
            ...
            return sorted(
                [
                    job["id"]
                    for job in test_info_list
                    if job["category_name"] == self.get_key()
                    and job["outcome"] == self.get_parent().get_key()
                ]
            )
...
class ResumeSessionBrowser:
    ...
    footer_text = [
        ("Press "),
        ("start", "<Enter>"),
        ("to resume selected session"),
    ]

    def _generate_details_widget(self):
        self.details_text = urwid.ListBox(
            urwid.SimpleListWalker(
                [
                    urwid.Text(
                        ("focus", " Keyboard Controls and Shortcuts "),
                        "center",
                    ),
                    urwid.Divider(),
                    urwid.Text("Select/Deselect                 Space"),
    ...
    def _change_details(self):
        ...
        details_w = urwid.AttrWrap(urwid.LineBox(self.details_text), "body")
        self.details_view = urwid.Overlay(
            details_w,
            self.frame,
            "center",
            ("relative", 80),
    ...
    def __init__(self, sessions):
        title = "Select the session to resume"
        self.header = urwid.Padding(urwid.Text(title), left=1)
        self.radio_button_group = []
        self.button_pile = None

        self._update_button_pile(sessions)
        listbox_content = [
            urwid.Divider(),
    ...
    def _update_button_pile(self, sessions):
        if sessions:
            contents = [
                urwid.AttrWrap(
                    urwid.RadioButton(
                        self.radio_button_group, session, state=False
                    ),
                    "buttn",
                    "buttnf",
                )
    ...
    def unhandled_input(self, key):
        ...
        if key == "esc":
            raise SystemExit("not selected")
        if key in ("d", "D"):
            self._change_details()
            self.loop.widget = self.details_view
            return True
...
class TestPlanBrowser:
    ...
    def __init__(self, title, test_plan_list, selection=None, resume_count=0):
        ...
        self.default_footer = urwid.AttrWrap(
            urwid.Columns(
                [
                    urwid.Padding(urwid.Text(self.footer_text), left=1),
                    urwid.Text(
                        "(R) Resume session ({} available)".format(
                            resume_count
                        ),
                        "center",
                    ),
    ...
    def _update_button_pile(self, tplist):
        if tplist:
            contents = [
                urwid.AttrWrap(
                    TestPlanButton(tp, self.radio_button_group),
                    "buttn",
                    "buttnf",
                )
                for tp in tplist
...
class ManifestQuestion(urwid.WidgetWrap):
    def __init__(self, question):
        ...
        if self._value_type == "bool":
            ...
            self._edit_widget = ManifestNaturalEdit("", self._value)
            self.display_widget = urwid.Columns(
                [
                    urwid.Padding(urwid.Text(question["name"]), left=2),
    ...
    def __init__(self, title, manifest):
        ...
        self.default_footer = urwid.AttrWrap(
            urwid.Columns(
                [
                    urwid.Padding(urwid.Text(self.footer_text), left=1),
                    urwid.Text(self.footer_shortcuts, "right"),
                ]
            ),
            "foot",
        )
...
def resume_dialog(duration):
    ...
    loop = urwid.MainLoop(frame, palette, handle_mouse=False, screen=Screen())
...
class ResumeInstead(Exception):
    """
    This is raised when selecting a test plan, but operator choses to resume
    previous session instead. This being an exception helps unwind the stack,
    and handle it where applicable.
    """
