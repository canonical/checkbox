...
"""
plainbox.impl.test_main
=======================

Test definitions for plainbox.impl.main module
"""
...
from plainbox.impl.main import main
...
class TestIO:
    ...
    def __enter__(self):
        ...
        if self._combined:
            self._fake_stderr = self._fake_stdout
        else:
    ...
    def __exit__(self, *exc):
        ...
        if self._combined:
            self._test_stderr = self._test_stdout
        else:
...
class TestIOTest(TestCase):
    ...
    def test_both_combined(self):
        with TestIO(combined=True) as io:
            ...
            print("Hello error", file=sys.stderr)
        self.assertEqual(io.stdout, "Hello output\nHello error\n")
        self.assertEqual(io.stderr, "Hello output\nHello error\n")
...
class TestMain(TestCase):
    ...
    def test_help(self):
        ...
        self.assertEqual(call.exception.args, (0,))
        expected = (
            "usage: plainbox [-h] [-v] [-u {headless,text,graphics}]\n"
            "\n"
            "optional arguments:\n"
            "  -h, --help            show this help message and exit\n"
            "  -v, --version         show program's version number and exit\n"
            "  -u {headless,text,graphics}, --ui {headless,text,graphics}\n"
            "                        select the UI front-end (defaults to"
            " auto)\n"
        )
        self.assertEqual(io.stdout, expected)

    def test_dummy(self):
        with TestIO() as io:
            main([])
        self.assertEqual(io.stdout, "I ran!\n")
