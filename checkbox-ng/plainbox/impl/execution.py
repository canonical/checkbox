...
class UnifiedRunner(IJobRunner):
    ...
    def validate_job(self, job):
        """
        Validate the input job if it was produced by a template,
        raises a ValueError if any validator fails or the job is invalid
        """
        # only do something if job was generated by a template
        try:
        ...
        unit_errors = []
        try:
            check_results = job.check()
        except MissingParam as m:
            unit_errors.append(
                "Failed to instantiate template '{}', resource '{}' is "
                "missing template parameter '{}'".format(
                    job.template_id,
                    job.id,
                    m.parameter,
                )
        ...
        for check_result in check_results:
            if check_result.severity == Severity.error:
                unit_errors.append(
                    "Failed to instantiate template '{}'. Validation failed "
                    "with message: {}".format(
                        job.template_id, check_result.message
                    )
                )
            elif check_results.severity == Severity.warning:
                logger.warning(
                    "Non-blocking problem encountered while instantiating "
                    "template '%s'. Validation warned of the following: %s",
                    job.template_id,
        ...
        if unit_errors:
            raise ValueError(
                [
                    "You generated this job via a template and it wasn't run because "
                    "it is invalid due to the following:"
                ]
                + unit_errors,
            )
