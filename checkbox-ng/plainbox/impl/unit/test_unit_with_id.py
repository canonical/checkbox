...
class UnitWithIdFieldValidationTests(UnitFieldValidationTests):
    ...
    def test_id__untranslatable(self):
        issue_list = self.unit_cls({"_id": "id"}, provider=self.provider).check()
    ...
    def test_id__unique(self):
        ...
        message_start = (
            "{} 'id', field 'id', clashes with 1 other unit," " look at: "
        ).format(unit.tr_unit())
    ...
    def test_id__without_namespace(self):
        unit = self.unit_cls({"id": "some_ns::id"}, provider=self.provider)
