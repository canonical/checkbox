...
class signal:
    ...
    def __init__(
        self, name_or_first_responder, pass_signal=False, signal_name=None
    ):
        """
    ...
    def __repr__(self):
        ...
        if len(self._listeners) > 0 and isinstance(
            ...
            return f"<signal name:{str(self._name)!r}>"
...
class SignalInterceptorMixIn:
    ...
    def assertSignalFired(self, signal, *args, **kwargs):
        ...
        self.assertIn(
            event,
            self._events_seen,
            f"\nSignal unexpectedly not fired: {event}\n",
        )
        return event
    ...
    def assertSignalNotFired(self, signal, *args, **kwargs):
        ...
        event = (signal, args, kwargs)
        self.assertNotIn(
            event, self._events_seen, f"\nSignal unexpectedly fired: {event}\n"
        )
    ...
    def assertSignalOrdering(self, *expected_events):
        ...
        """
        expected_order = [
            self._events_seen.index(event) for event in expected_events
        ]
