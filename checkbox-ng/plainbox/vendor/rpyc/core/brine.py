...
TAG_FSET = b"\x1a"
TAG_COMPLEX = b"\x1b"
IMM_INTS = {i: bytes([i + 0x50]) for i in range(-0x30, 0xA0)}
...
_dump_registry = {}
_load_registry = {}
IMM_INTS_LOADER = {v: k for k, v in IMM_INTS.items()}
...
def dumpable(obj):
    ...
    if type(obj) is slice:
        return (
            dumpable(obj.start) and dumpable(obj.stop) and dumpable(obj.step)
        )
    return False
