...
class BaseNetref(with_metaclass(NetrefMetaclass, object)):
    ...
    def __exit__(self, exc, typ, tb):
        return syncreq(
            self, consts.HANDLE_CTXEXIT, exc
        )  # can't pass type nor traceback
    ...
    def __instancecheck__(self, other):
        ...
        if isinstance(other, BaseNetref):
            if self.____id_pack__[2] != 0:
                ...
                return syncreq(
                    self, consts.HANDLE_INSTANCECHECK, other.____id_pack__
                )
        else:
            if self.____id_pack__[2] == 0:
                ...
                return isinstance(
                    other, type(self).__dict__["__class__"].instance
                )
            else:
                raise TypeError(
                    "isinstance() arg 2 must be a class, type, or tuple of classes and types"
...
class NetrefClass:
    ...
    def __get__(self, netref_instance, netref_owner):
        ...
        return (
            self.owner
            if netref_instance.____id_pack__[2] == 0
            else self.instance
        )
...
def class_factory(id_pack, methods):
    ...
    netref_name = (
        class_descriptor.owner.__name__
...
for _builtin in _builtin_types:
    ...
    builtin_classes_cache[_name_pack] = class_factory(
