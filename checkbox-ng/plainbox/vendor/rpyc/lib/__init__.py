...
def setup_logger(quiet=False, logfile=None, namespace=None):
    ...
    return logging.getLogger(
        "rpyc" if namespace is None else f"rpyc.{namespace}"
    )
...
def spawn(*args, **kwargs):
    ...
    thread = threading.Thread(
        name=f"{SPAWN_THREAD_PREFIX}-{str_id_pack}",
        target=func,
        args=args,
        kwargs=kwargs,
    )
...
def get_id_pack(obj):
    ...
    if hasattr(obj, "____id_pack__"):
        ...
        if isinstance(obj, type):  # module
            ...
            if inspect.ismodule(obj) and obj.__name__ != "module":
                if obj.__name__ in sys.modules:
                    ...
                    name_pack = f"{obj.__class__.__module__}.{obj.__name__}"
        ...
        name_pack = f"{obj.__class__.__module__}.{obj.__class__.__name__}"
        return (name_pack, id(type(obj)), id(obj))
    else:
        name_pack = f"{obj.__module__}.{obj.__name__}"
