...
def service(cls):
    ...
    for attr_name, attr_obj in inspect.getmembers(
        cls
    ):  # rebind exposed decorated attributes
        exposed_prefix = getattr(attr_obj, "__exposed__", False)
        if exposed_prefix and not inspect.iscode(
            attr_obj
        ):  # exclude the implementation
            renamed = exposed_prefix + attr_name
            if inspect.isclass(
                attr_obj
            ):  # recurse exposed objects such as a class
...
def exposed(arg):
    ...
    if isinstance(arg, str):
        ...
        raise TypeError(
            "rpyc.exposed expects a callable object, descriptor, or string"
        )
