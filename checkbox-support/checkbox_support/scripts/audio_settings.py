...
def _guess_hdmi_profile(pactl_list):
    ...
    for card, ports in hdmi_ports.items():
        for card_port in ports:
            logging.info(f"Card #{card} Port: {card_port}")
    ...
    if available_port:
        ...
        profile = min(
            [
                p
                for p in port.profile_list
                if ("stereo" in p) or ("Hdmi" in p) or ("HiFi" in p)
            ],
            key=len,
        )
        logging.info("[ Selected profile ]".center(80, "="))
        logging.info(f"Card #{card} Profile: {profile}")
        return (card, profile)
    else:
        return (None, None)
...
def set_profile_hdmi():
    ...
    try:
        ...
        logging.error(f"Failed setting audio output to:{profile}: {error}")
    ...
    for input_index in input_indexes:
        try:
            with open(os.devnull, "wb") as DEVNULL:
                check_call(
                    ["pactl", "move-sink-input", input_index, name],
                    stdout=DEVNULL,
                )
        except CalledProcessError:
            logging.error(f"Failed to move input {input_index} to sink {name}")
            sys.exit(1)


def get_audio_settings(type, name="default"):
    if name == "default":
        pactl_status = check_output(
            ["pactl", "info"], universal_newlines=True, env=unlocalized_env()
        )
...
def store_audio_settings(file):
    ...
    try:
        ...
        logging.error(f"Failed to save settings: {sys.exc_info()[1]}")
        sys.exit(1)

    for type in TYPES:
        audio_settings = get_audio_settings(type)
        print(
            "default_{}: {}".format(type, audio_settings["name"]),
            file=settings_file,
        )
        print(
            "{}_muted: {}".format(type, audio_settings["muted"].strip()),
            file=settings_file,
        )
        print(
            "{}_volume: {}%".format(type, str(audio_settings["max_volume"])),
            file=settings_file,
        )
        ...
        for name in names:
            ...
            if device in name and DIRECTIONS[type] in name:
                try:
                    ...
                    with open(os.devnull, "wb") as DEVNULL:
                        check_call(
                            ["pactl", f"set-default-{type}", name],
                            stdout=DEVNULL,
                        )
                except CalledProcessError:
                    logging.error(f"Failed to set default {type}")
                    sys.exit(1)
                ...
                try:
                    ...
                    logging.error(f"Failed to set mute for {name}")
                    sys.exit(1)
                ...
                try:
                    ...
                    logging.error(f"Failed to set volume for {name}")
                    sys.exit(1)
...
def restore_audio_settings(file):
    ...
    for type in TYPES:
        ...
        try:
            name = settings_file[settings_file.index(f"default_{type}:") + 1]
            muted = settings_file[settings_file.index(f"{type}_muted:") + 1]
            volume = settings_file[settings_file.index(f"{type}_volume:") + 1]
        except ValueError:
            logging.error(
        ...
        try:
            ...
            logging.error(f"Failed to restore default {name}")
            return 1
        ...
        try:
            check_call(["pactl", f"set-{type}-mute", name, muted])
        except CalledProcessError:
            logging.error(f"Failed to restore mute for {name}")
            return 1

        try:
            check_call(["pactl", f"set-{type}-volume", name, volume])
        except CalledProcessError:
            logging.error(f"Failed to restore volume for {name}")
            return 1
...
def main():
    ...
    parser.add_argument(
        "-d", "--device", help="The device to apply the new settings to."
    )
    parser.add_argument(
        "-m",
        "--mute",
    ...
    if args.action in ["store", "restore"]:
        ...
        profiles_file = f"{args.file}.profiles"
    ...
    if args.action == "store":
        ...
        if restore_profiles_settings(profiles_file) or restore_audio_settings(
            settings_file
        ):
            return 1
        ...
        logging.error(f"{args.action} is not a valid action")
        return 1
