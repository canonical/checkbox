...
def get_available_fwts_tests():
    ...
    if result.returncode != 0:
        raise RuntimeError(f"FWTS command failed: {stderr.decode()}")
...
def average_times(runs):
    ...
    if sleep_run_count != len(runs):
        print(
            (
                "Warning: Time to sleep was only reported {} times out of {} "
                "iterations!"
            ).format(sleep_run_count, len(runs))
        )
    if resume_run_count != len(runs):
        print(
            (
                "Warning: Time to resume was only reported {} times out of {} "
                "iterations!"
            ).format(resume_run_count, len(runs))
        )
    print()
    if sleep_run_count:
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["critical", "high", "medium", "low", "none", "aborted"],
        help=(
            "Specify the FWTS failure level that will "
    ...
    group.add_argument(
        ...
        action="store",
        help=(
            "Perform sleep test(s) using the additional\n"
    ...
    group.add_argument(
        "--hwe", action="store_true", help="Run HWE concerned tests in fwts"
    )
    group.add_argument(
        "--qa", action="store_true", help="Run QA concerned tests in fwts"
    )
    group.add_argument(
        "--server",
        action="store_true",
    ...
    if critical_fails:
        ...
        if (
            args.quiet and fail_priority <= fail_levels["FAILED_CRITICAL"]
        ) or not args.quiet:
            print(
                " WARNING: The following test cases were reported as"
                " critical\n"
    ...
    if high_fails:
        ...
        if (
            args.quiet and fail_priority <= fail_levels["FAILED_HIGH"]
        ) or not args.quiet:
            print(
    ...
    if medium_fails:
        ...
        if (
            args.quiet and fail_priority <= fail_levels["FAILED_MEDIUM"]
        ) or not args.quiet:
            print(
                " WARNING: The following test cases were reported as"
                " medium\n"
    ...
    if low_fails:
        ...
        if (
            args.quiet and fail_priority <= fail_levels["FAILED_LOW"]
        ) or not args.quiet:
            print(
    ...
    if passed:
        ...
        if (
            args.quiet and fail_priority <= fail_levels["FAILED_NONE"]
        ) or not args.quiet:
            for test in passed:
                print(" - " + test)
    if skipped:
    ...
    if aborted:
        ...
        if (
            args.quiet and fail_priority <= fail_levels["FAILED_ABORTED"]
        ) or not args.quiet:
