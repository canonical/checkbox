...
class HumanReadableBytesTests(unittest.TestCase):
    ...
    def test_parsing_all_equal_four(self):
        fours = [
            HumanReadableBytes(4),
            4,
            HumanReadableBytes("4"),
            HumanReadableBytes("4B"),
            HumanReadableBytes("4b"),
            int("4"),
        ]
    ...
    def test_parsin_all_minus_2kibi(self):
        values = [
            HumanReadableBytes("-2ki"),
            -2048,
            HumanReadableBytes("-2048"),
            HumanReadableBytes("-2KiB"),
        ]
        self.assertEqual(all(values[0] == val for val in values), True)

    def test_four_megs(self):
        values = [
            HumanReadableBytes("4mB"),
            int("4" + "0" * 6),
            HumanReadableBytes(4000000),
            4000000,
        ]
        self.assertEqual(all(values[0] == val for val in values), True)
