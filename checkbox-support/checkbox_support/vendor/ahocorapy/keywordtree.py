...
class KeywordTree(object):
    ...
    def add(self, keyword):
        ...
        if self._finalized:
            raise ValueError(
                "KeywordTree has been finalized." + " No more keyword additions allowed"
            )
    ...
    def search_all(self, text):
        ...
        if not self._finalized:
            raise ValueError(
                "KeywordTree has not been finalized."
    ...
    def __getstate__(self):
        ...
        while todo_list:
            state = todo_list.pop()
            transitions = {
                key: value.identifier for key, value in state.transitions.items()
            }
            state_list[state.identifier] = {
                ...
                "longest_strict_suffix": (
                    state.longest_strict_suffix.identifier
                    if state.longest_strict_suffix is not None
                    else None
                ),
            ...
            for child in state.transitions.values():
                if (
                    len(state_list) <= child.identifier
                    or not state_list[child.identifier]
                ):
    ...
    def __setstate__(self, state):
        ...
        for idx, serialized_state in enumerate(state["states"]):
            ...
            deserialized_state.transitions = {
                key: states[value]
                for key, value in serialized_state["transitions"].items()
            }
