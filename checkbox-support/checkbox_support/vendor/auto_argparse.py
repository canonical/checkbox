...
def _parse_docstring(doc):
    ...
    return_re = re.compile(r":returns(\s[A-Za-z_][A-Za-z0-9_]*)?:(.*)")
    for line in lines:
        param_match = param_re.match(line)
        if param_match:
            param_type, param_name, description = param_match.groups()
            param_type = {"str": str, "int": int, "float": float, "bool": bool}.get(
                param_type.strip() if param_type else None
            )
...
class AutoArgParser(argparse.ArgumentParser):
    ...
    def run(self, obj=None):
        ...
        if not self._picked_method:
            raise SystemExit("No sub-command chosen!\n\n{}".format(self.format_help()))
