...
def open_dev(bt_device_id):
    ...
    if not isinstance(bt_device_id, str):
        bt_device_id = f"ubt{bt_device_id}hci"
...
def send_req(
    _socket, _group_field, _command_field, _event, _rlen, _params, _timeout
):
    """Support for HCI 5 has not been implemented yet for FreeBSD, pull requests are wellcome"""
    raise NotImplementedError(
        "send_req has not been implemented yet for FreeBSD"
    )
