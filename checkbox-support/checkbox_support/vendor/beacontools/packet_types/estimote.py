...
class EstimoteTelemetryFrameA:
    ...
    def __init__(self, data, protocol_version):
        ...
        self._acceleration = tuple(
            [v * 2 / 127.0 for v in sub["acceleration"]]
        )
        # motion states
        self._previous_motion_state = self.parse_motion_state(
            sub["previous_motion"]
        )
        self._current_motion_state = self.parse_motion_state(
            sub["current_motion"]
        )
        ...
        if self.protocol_version == 2:
            self._has_firmware_error = (
                (sub["combined_fields"][0] & 0b00000100) >> 2
            ) == 1
            self._has_clock_error = (
                (sub["combined_fields"][0] & 0b00001000) >> 3
            ) == 1
        elif self.protocol_version == 1:
            self._has_firmware_error = (
                sub["combined_fields"][1] & 0b00000001
            ) == 1
            self._has_clock_error = (
                (sub["combined_fields"][1] & 0b00000010) >> 1
            ) == 1
        else:
    ...
    def __str__(self):
        return (
            "EstimoteTelemetryFrameA<identifier: %s, protocol_version: %u>"
            % (self.identifier, self.protocol_version)
        )


class EstimoteTelemetryFrameB:
    ...
    def __init__(self, data, protocol_version):
        ...
        if sub["magnetic_field"] == [-1, -1, -1]:
            ...
            self._magnetic_field = tuple(
                [v / 128.0 for v in sub["magnetic_field"]]
            )
        # ambient light
        ...
        uptime_number = ((sub["combined_fields"][1] & 0b00001111) << 8) | sub[
            "combined_fields"
        ][0]
        ...
        if self._protocol_version == 0:
            ...
            self._battery_level = (
                None if sub["battery_level"] == 0xFF else sub["battery_level"]
            )
            self._has_clock_error = None
            self._has_firmware_error = None
    ...
    def __str__(self):
        return (
            "EstimoteTelemetryFrameB<identifier: %s, protocol_version: %u>"
            % (self.identifier, self.protocol_version)
        )


class EstimoteNearable:
    ...
    def __str__(self):
        return "EstimoteNearable<identifier: %s>" % self.identifier
