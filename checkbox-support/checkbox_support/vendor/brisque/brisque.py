...
class BRISQUE:
    ...
    def score(self, path):
        ...
        downscaled_image = cv2.resize(gray_image, None, fx=1 / 2, fy=1 / 2,
    ...
    def generalized_gaussian_dist(self, x, alpha, sigma):
        beta = sigma * np.sqrt(math.gamma(1 / alpha)
                               / math.gamma(3 / alpha))
    ...
    def calculate_pair_product_coeff(self, mscn_coeff):
        ...
        od['main_diagonal'] = (mscn_coeff[:-1, :-1] *
                               mscn_coeff[1:, 1:])
        od['secondary_diagonal'] = (mscn_coeff[1:, :-1] *
                                    mscn_coeff[:-1, 1:])
        return od

    def asymmetric_generalized_gaussian(self, x, nu, sigma_l, sigma_r):
        def beta(sigma):
            return sigma * np.sqrt(math.gamma(1 / nu) /
                                   math.gamma(3 / nu))

        def f_par(x, sigma):
            return coefficient * np.exp(-(x / beta(sigma)) ** nu)

        coefficient = nu / ((beta(sigma_l) + beta(sigma_r)) *
                            math.gamma(1 / nu))
    ...
    def asymmetric_generalized_gaussian_fit(self, x):
        ...
        def estimate_mean(alpha, sigma_l, sigma_r):
            return (sigma_r - sigma_l) * constant * (math.gamma(2 / alpha) /
                                                     math.gamma(1 / alpha))
    ...
    def calculate_brisque_features(self, image, kernel_size=7, sigma=7 / 6):
        ...
        features = [calculate_features(coeff_name=name, coeff=cf)
    ...
    def calculate_image_quality_score(self, brisque_features):
        ...
        return svmutil.libsvm.svm_predict_probability(self.model, x,
                                                      prob_estimates)
