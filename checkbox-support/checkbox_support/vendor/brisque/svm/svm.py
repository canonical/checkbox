...
def gen_svm_nodearray(xi, feature_max=None, isKernel=False):
    ...
    if (
        ...
        and len(xi) == 2
        and isinstance(xi[0], scipy.ndarray)
        and isinstance(xi[1], scipy.ndarray)
    ):  # for a sparse vector
        if not isKernel:
...
@jit
def csr_to_problem_jit(
    l, x_val, x_ind, x_rowptr, prob_val, prob_ind, prob_rowptr, indx_start
):
    for i in range(l):
...
class svm_problem(Structure):
    ...
    def __init__(self, y, x, isKernel=False):
        if (not isinstance(y, (list, tuple))) and (
            ...
            raise TypeError(f"type of y: {type(y)} is not supported!")
