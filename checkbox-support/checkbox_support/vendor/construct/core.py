...
from checkbox_support.vendor.construct.version import *
...
def stream_read(stream, length, path):
    if length < 0:
        raise StreamError(
            "length must be non-negative, found %s" % length, path=path
        )
    try:
...
def stream_read_entire(stream, path):
    try:
        ...
        raise StreamError(
            "stream.read() failed when reading until EOF", path=path
        )
...
def stream_write(stream, data, length, path):
    if not isinstance(data, bytestringtype):
        raise StringError(
            f"given non-bytes value, perhaps unicode? {data!r}",
            path=path,
        )
    if length < 0:
        raise StreamError(
    ...
    if len(data) != length:
        raise StreamError(
            "bytes object of wrong length, expected %d, found %d"
            % (length, len(data)),
            path=path,
        )
    try:
        ...
        raise StreamError(f"stream.write() failed, given {data!r}", path=path)
    if written != length:
        raise StreamError(
...
def stream_seek(stream, offset, whence, path):
    try:
        ...
        raise StreamError(
            "stream.seek() failed, offset {}, whence {}".format(
                offset, whence
            ),
            path=path,
        )
...
class Construct:
    ...
    * `__setstate__`
    ...
    def compile(self, filename=None):
        ...
        """.format(self._compileparse(code)))
    ...
    def benchmark(self, sampledata, filename=None):
        ...
        try:
            ...
            runs = min(1000, max(1, int(1.0 / parsetime)))
            if runs > 1:
                parsetime = (
                    timeit(lambda: self.parse(sampledata), number=runs) / runs
                )
            parsetime = f"{parsetime:.10f} sec/call"

            self.build(sampleobj)
            buildtime = timeit(lambda: self.build(sampleobj), number=1)
            runs = min(1000, max(1, int(1.0 / buildtime)))
            if runs > 1:
                buildtime = (
                    timeit(lambda: self.build(sampleobj), number=runs) / runs
                )
            buildtime = f"{buildtime:.10f} sec/call"
            ...
            runs = min(1000, max(1, int(1.0 / parsetime2)))
            if runs > 1:
                parsetime2 = (
                    timeit(lambda: compiled.parse(sampledata), number=runs)
                    / runs
                )
            parsetime2 = f"{parsetime2:.10f} sec/call"
    ...
    def export_ksy(self, schemaname="unnamed_schema", filename=None):
        ...
        gen = KsyGen()
        main = dict(
            meta=dict(id=schemaname),
            seq=self._compileseq(gen),
    ...
    def _compileseq(self, ksy, bitwise=False, recursion=0):
        ...
        try:
            ...
            return [
                dict(
                    id="x",
                    **self._compilefulltype(ksy, bitwise, recursion + 1),
                )
            ]
    ...
    def _compileprimitivetype(self, ksy, bitwise=False, recursion=0):
        ...
        try:
            ...
            ksy.types[name] = dict(
                seq=self._compileseq(ksy, bitwise, recursion + 1)
            )
            return name
    ...
    def _compilefulltype(self, ksy, bitwise=False, recursion=0):
        ...
        try:
            ...
            return dict(
                type=self._compileprimitivetype(ksy, bitwise, recursion + 1)
            )
    ...
    def __mul__(self, other):
        ...
        raise ConstructError(
            "operator * can only be used with string or lambda"
        )
    ...
    def __getitem__(self, count):
        ...
        raise ConstructError(
            "subcon[N] syntax expects integer or context lambda"
        )
    ...
    def _decode(self, obj, context, path):
        if not self._validate(obj, context, path):
            raise ValidationError(
                f"object failed validation: {obj}", path=path
            )
        return obj
...
class Bytes(Construct):
    ...
    def _emitparse(self, code):
        return f"read_bytes(io, {self.length})"
...
def Bitwise(subcon):
    ...
    try:
        size = subcon.sizeof()
        macro = Transformed(
            subcon, bytes2bits, size // 8, bits2bytes, size // 8
        )
    except SizeofError:
        macro = Restreamed(
            subcon, bytes2bits, 1, bits2bytes, 8, lambda n: n // 8
        )
...
def Bytewise(subcon):
    ...
    try:
        ...
        macro = Restreamed(
            subcon, bits2bytes, 8, bytes2bits, 1, lambda n: n * 8
        )
...
class FormatField(Construct):
    ...
    def __init__(self, endianity, format):
        ...
        if format not in list("fdBHLQbhlqe"):
            raise FormatFieldError(
                "format must be like: f d B H L Q b h l q e", format
            )
    ...
    def _emitparse(self, code):
        ...
        code.append(f"{fname} = struct.Struct({self.fmtstr!r})")
        return f"{fname}.unpack(read_bytes(io, {self.length}))[0]"

    def _emitprimitivetype(self, ksy, bitwise):
        endianity, format = self.fmtstr
        signed = format.islower()
        swapped = (endianity == "<") or (
            endianity == "=" and sys.byteorder == "little"
        )
        if format in "bhlqBHLQ":
            if bitwise:
                assert not signed
                assert not swapped
                return f"b{8 * self.length}"
            else:
                return "{}{}{}".format(
                    "s" if signed else "u",
                    self.length,
                    "le" if swapped else "be",
                )
        if format in "fd":
            assert not bitwise
            return "f{}{}".format(self.length, "le" if swapped else "be")
...
class BytesInteger(Construct):
    ...
    def _build(self, obj, stream, context, path):
        if not isinstance(obj, integertypes):
            raise IntegerError(f"value {obj!r} is not an integer", path=path)
        if obj < 0 and not self.signed:
            raise IntegerError(
                f"value {obj!r} is negative, but field is not signed",
                path=path,
            )
        length = self.length
        if callable(length):
    ...
    def _emitparse(self, code):
        return "bytes2integer(read_bytes(io, {}){}, {})".format(
            self.length, "[::-1]" if self.swapped else "", self.signed
        )
...
class BitsInteger(Construct):
    ...
    def _emitparse(self, code):
        return "bits2integer(read_bytes(io, {}){}, {})".format(
            self.length, "[::-1]" if self.swapped else "", self.signed
        )
...
class VarInt(Construct):
    ...
    def _build(self, obj, stream, context, path):
        ...
        if obj < 0:
            raise IntegerError(
                f"varint cannot build from negative number: {obj!r}",
                path=path,
            )
        x = obj
        while x > 0b01111111:
            stream_write(
                stream, int2byte(0b10000000 | (x & 0b01111111)), 1, path
            )
            x >>= 7
...
def encodingunit(encoding):
    ...
    if encoding not in possiblestringencodings:
        raise StringError(
            "encoding {!r} not found among {!r}".format(
                encoding, possiblestringencodings
            )
        )
...
class StringEncoded(Adapter):
    ...
    def _emitparse(self, code):
        return "({}).decode({!r})".format(
            self.subcon._compileparse(code), self.encoding
        )
...
def PaddedString(length, encoding):
    ...
    macro = StringEncoded(
        FixedSized(
            length, NullStripped(GreedyBytes, pad=encodingunit(encoding))
        ),
        encoding,
    )
...
def PascalString(lengthfield, encoding):
    ...
    def _emitseq(ksy, bitwise):
        return [
            dict(
                id="lengthfield",
                type=lengthfield._compileprimitivetype(ksy, bitwise),
            ),
...
class EnumIntegerString(str):
    ...
    def __repr__(self):
        return "EnumIntegerString.new({}, {})".format(
            self.intvalue, str.__repr__(self)
        )
...
class Enum(Adapter):
    ...
    def __init__(self, subcon, *merge, **mapping):
        ...
        self.encmapping = {
            EnumIntegerString.new(v, k): v for k, v in mapping.items()
        }
        self.decmapping = {
            v: EnumIntegerString.new(v, k) for k, v in mapping.items()
        }
    ...
    def _encode(self, obj, context, path):
        try:
            ...
            raise MappingError(
                f"building failed, no mapping for {obj!r}", path=path
            )

    def _emitparse(self, code):
        fname = "factory_%s" % code.allocateId()
        code.append(f"{fname} = {self.decmapping!r}")
        return "reuse({}, lambda x: {}.get(x, EnumInteger(x)))".format(
            self.subcon._compileparse(code), fname
        )
...
class BitwisableString(str):
    ...
    def __or__(self, other):
        return BitwisableString(f"{self}|{other}")
    ...
    def _encode(self, obj, context, path):
        try:
            ...
            if isinstance(obj, dict):
                ...
                return flags
            raise MappingError(
                f"building failed, unknown object: {obj!r}", path=path
            )
        except KeyError:
            raise MappingError(
                f"building failed, unknown label: {obj!r}", path=path
            )

    def _emitparse(self, code):
        return "reuse({}, lambda x: Container({}))".format(
            self.subcon._compileparse(code),
            ", ".join(f"{k}=bool(x & {v})" for k, v in self.flags.items()),
        )
...
class Mapping(Adapter):
    ...
    def _decode(self, obj, context, path):
        try:
            ...
            raise MappingError(
                f"parsing failed, no decoding mapping for {obj!r}",
                path=path,
            )
    ...
    def _encode(self, obj, context, path):
        try:
            ...
            raise MappingError(
                f"building failed, no encoding mapping for {obj!r}",
                path=path,
            )
...
class Struct(Construct):
    ...
    def __init__(self, *subcons, **subconskw):
        super().__init__()
        self.subcons = list(subcons) + list(
            k / v for k, v in subconskw.items()
        )
        self._subcons = Container(
            (sc.name, sc) for sc in self.subcons if sc.name
        )
    ...
    def _emitparse(self, code):
        ...
        for sc in self.subcons:
            ...
            """.format(
                (
                    f"result[{sc.name!r}] = this[{sc.name!r}] = "
                    if sc.name
                    else ""
                ),
...
class Array(Subconstruct):
    ...
    def _emitparse(self, code):
        return "ListContainer((this.__setitem__('_index',i),({}))[1] for i in range({}))".format(
            self.subcon._compileparse(code), self.count
        )
...
class GreedyRange(Subconstruct):
    ...
    def _emitfulltype(self, ksy, bitwise):
        return dict(
            type=self.subcon._compileprimitivetype(ksy, bitwise), repeat="eos"
        )
...
class RepeatUntil(Subconstruct):
    ...
    def _build(self, obj, stream, context, path):
        ...
        for i, e in enumerate(obj):
            ...
            raise RepeatError(
                "expected any item to match predicate, when building",
                path=path,
            )
        return retlist
        ...
        block = """
            def {}(io, this):
                ...
                for i in itertools.count():
                    ...
                    if ({}):
                        return list_
        """.format(fname, self.subcon._compileparse(code), self.predicate)
        code.append(block)
        return f"{fname}(io, this)"
...
class Const(Subconstruct):
    ...
    def __init__(self, value, subcon=None):
        if subcon is None:
            if not isinstance(value, bytestringtype):
                raise StringError(
                    "given non-bytes value, perhaps unicode? {!r}".format(
                        value
                    )
                )
    ...
    def _parse(self, stream, context, path):
        ...
        if obj != self.value:
            raise ConstError(
                "parsing expected {!r} but parsed {!r}".format(
                    self.value, obj
                ),
                path=path,
            )
    ...
    def _build(self, obj, stream, context, path):
        if obj not in (None, self.value):
            raise ConstError(
                "building expected None or {!r} but got {!r}".format(
                    self.value, obj
                ),
                path=path,
            )
    ...
    def _emitparse(self, code):
        code.append("""
            def parse_const(value, expected):
                ...
                return value
        """)
        return "parse_const({}, {!r})".format(
            self.subcon._compileparse(code), self.value
        )
...
class Default(Subconstruct):
    ...
    def _build(self, obj, stream, context, path):
        obj = (
            (self.value(context) if callable(self.value) else self.value)
            if obj is None
            else obj
        )
        return self.subcon._build(obj, stream, context, path)
...
class Check(Construct):
    ...
    def _emitparse(self, code):
        ...
        """)
        return f"parse_check({self.func})"
...
class Error(Construct):
    ...
    def _parse(self, stream, context, path):
        raise ExplicitError(
            "Error field was activated during parsing", path=path
        )

    def _build(self, obj, stream, context, path):
        raise ExplicitError(
            "Error field was activated during building", path=path
        )
...
class FocusedSeq(Construct):
    ...
    def _build(self, obj, stream, context, path):
        ...
        for i, sc in enumerate(self.subcons):
            buildret = sc._build(
                obj if sc.name == parsebuildfrom else None,
                stream,
                context,
                path,
            )
...
class NamedTuple(Adapter):
    ...
    def _emitparse(self, code):
        fname = "factory_%s" % code.allocateId()
        code.append("""
            {} = collections.namedtuple({!r}, {!r})
        """.format(fname, self.tuplename, self.tuplefields))
        if isinstance(self.subcon, Struct):
            return f"{fname}(**({self.subcon._compileparse(code)}))"
        if isinstance(self.subcon, (Sequence, Array, GreedyRange)):
            return f"{fname}(*({self.subcon._compileparse(code)}))"
...
class Union(Construct):
    ...
    def _parse(self, stream, context, path):
        ...
        if parsefrom is not None:
            stream_seek(
                stream, forwards[parsefrom], 0, path
            )  # raises KeyError
        return obj
    ...
    def _emitparse(self, code):
        if callable(self.parsefrom):
            raise NotImplementedError(
                "Union does not compile non-constant parsefrom"
            )
        ...
        if isinstance(self.parsefrom, int):
            ...
            skipfallback = True
            skipforward = (
                self.subcons[index].sizeof() == self.subcons[-1].sizeof()
            )
        ...
        for i, sc in enumerate(self.subcons):
            block += """
                {}{}
            """.format(
                "this[%r] = " % sc.name if sc.name else "",
                sc._compileparse(code),
            )
    ...
    def _build(self, obj, stream, context, path):
        for sc in self.subcons:
            try:
                ...
                stream_write(stream, data, len(data), path)
                return obj
        raise SelectError(f"no subconstruct matched: {obj}", path=path)
...
def If(condfunc, subcon):
    ...
    def _emitfulltype(ksy, bitwise):
        return dict(
            ...
            if_=repr(condfunc).replace("this.", ""),
        )
...
class IfThenElse(Construct):
    ...
    def __init__(self, condfunc, thensubcon, elsesubcon):
        ...
        self.flagbuildnone = (
            thensubcon.flagbuildnone and elsesubcon.flagbuildnone
        )
    ...
    def _emitseq(self, ksy, bitwise):
        return [
            dict(
                id="thenvalue",
                type=self.thensubcon._compileprimitivetype(ksy, bitwise),
                if_=repr(self.condfunc).replace("this.", ""),
            ),
            dict(
                id="elsesubcon",
                type=self.elsesubcon._compileprimitivetype(ksy, bitwise),
                if_=repr(~self.condfunc).replace("this.", ""),
            ),
        ]
    ...
    def _emitparse(self, code):
        ...
        code.append(
            "{} = {{{}}}".format(
                fname,
                ", ".join(
                    "{!r} : lambda io,this: {}".format(
                        key, sc._compileparse(code)
                    )
        ...
        code.append(
            "{} = lambda io,this: {}".format(
                defaultfname, self.default._compileparse(code)
            )
        )
        return "{}.get({}, {})(io, this)".format(
            fname, self.keyfunc, defaultfname
        )
...
class StopIf(Construct):
    ...
    def _emitparse(self, code):
        ...
        """)
        return f"parse_stopif({self.condfunc})"
...
class Aligned(Subconstruct):
    ...
    def _parse(self, stream, context, path):
        modulus = (
            self.modulus(context) if callable(self.modulus) else self.modulus
        )
        if modulus < 2:
    ...
    def _emitparse(self, code):
        code.append("""
            def parse_pointer(io, offset, func):
                ...
                return obj
        """)
        return "parse_pointer(io, {}, lambda: {})".format(
            self.offset, self.subcon._compileparse(code)
        )

    def _emitprimitivetype(self, ksy, bitwise):
        offset = (
            self.offset.__getfield__()
            if callable(self.offset)
            else self.offset
        )
        name = "instance_%s" % ksy.allocateId()
        ksy.instances[name] = dict(
            pos=offset, **self.subcon._compilefulltype(ksy, bitwise)
        )
        return name
        ...
        code.append("""
            def parse_peek(io, func):
                ...
                try:
                    ...
                    io.seek(fallback)
        """)
        return "parse_peek(io, lambda: {})".format(
            self.subcon._compileparse(code)
        )
...
class RawCopy(Subconstruct):
    ...
    def _build(self, obj, stream, context, path):
        ...
        raise RawCopyError(
            "RawCopy cannot build, both data and value keys are missing",
            path=path,
        )
...
def BitsSwapped(subcon):
    ...
    try:
        size = subcon.sizeof()
        return Transformed(
            subcon, swapbitsinbytes, size, swapbitsinbytes, size
        )
    except SizeofError:
        return Restreamed(
            subcon, swapbitsinbytes, 1, swapbitsinbytes, 1, lambda n: n
        )
...
class Prefixed(Subconstruct):
    ...
    def _emitparse(self, code):
        ...
        return "restream(read_bytes(io, ({})-({})), lambda io: {})".format(
            self.lengthfield._compileparse(code),
            sub,
            self.subcon._compileparse(code),
        )
    ...
    def _emitseq(self, ksy, bitwise):
        return [
            dict(
                id="lengthfield",
                type=self.lengthfield._compileprimitivetype(ksy, bitwise),
            ),
            dict(
                id="data",
                size="lengthfield",
...
def PrefixedArray(countfield, subcon):
    ...
    def _emitparse(code):
        return "ListContainer(({}) for i in range({}))".format(
            subcon._compileparse(code), countfield._compileparse(code)
        )
    ...
    def _emitseq(ksy, bitwise):
        return [
            dict(
                id="countfield",
                type=countfield._compileprimitivetype(ksy, bitwise),
            ),
            dict(
                id="data",
                type=subcon._compileprimitivetype(ksy, bitwise),
...
class FixedSized(Subconstruct):
    ...
    def _emitparse(self, code):
        return "restream(read_bytes(io, {}), lambda io: {})".format(
            self.length, self.subcon._compileparse(code)
        )

    def _emitfulltype(self, ksy, bitwise):
        return dict(
            size=repr(self.length).replace("this.", ""),
...
class NullTerminated(Subconstruct):
    ...
    def _emitfulltype(self, ksy, bitwise):
        if len(self.term) > 1:
            raise NotImplementedError
        return dict(
            terminator=byte2int(self.term),
            include=self.include,
            consume=self.consume,
            eos_error=self.require,
            **self.subcon._compilefulltype(ksy, bitwise),
        )
...
class RestreamData(Subconstruct):
    ...
    def _emitparse(self, code):
        return "restream({!r}, lambda io: {})".format(
            self.datafunc, self.subcon._compileparse(code)
        )
...
class Transformed(Subconstruct):
    ...
    def __init__(
        self, subcon, decodefunc, decodeamount, encodefunc, encodeamount
    ):
        super().__init__(subcon)
...
class Restreamed(Subconstruct):
    ...
    def _sizeof(self, context, path):
        if self.sizecomputer is None:
            raise SizeofError(
                "Restreamed cannot calculate size without a sizecomputer",
                path=path,
            )
        else:
...
class ProcessXor(Subconstruct):
    ...
    def _parse(self, stream, context, path):
        ...
        if not isinstance(pad, (integertypes, bytestringtype)):
            raise StringError(
                "ProcessXor needs integer or bytes pad", path=path
            )
        ...
        if isinstance(pad, bytestringtype):
            if not (len(pad) <= 64 and pad == bytes(len(pad))):
                data = integers2bytes(
                    (b ^ p)
                    for b, p in zip(
                        iterateints(data), itertools.cycle(iterateints(pad))
                    )
                )
        if self.subcon is GreedyBytes:
            return data
...
class ProcessRotateLeft(Subconstruct):
    ...
    precomputed_single_rotations = {
        amount: [
            (i << amount) & 0xFF | (i >> (8 - amount)) for i in range(256)
        ]
    ...
    def _parse(self, stream, context, path):
        ...
        if amount == 0:
            ...
            data = integers2bytes(
                (data_ints[i + k1] << amount1) & 0xFF
                | (data_ints[i + k2] >> amount2)
                for i in range(0, len(data), group)
...
class LazyContainer(dict):
    ...
    def __getitem__(self, index):
        ...
        stream_seek(
            self._stream, self._offsets[index], 0, self._path
        )  # KeyError
...
class LazyListContainer(list):
    ...
    def __getitem__(self, index):
        ...
        parseret = self._subcon._parsereport(
            self._stream, self._context, self._path
        )
        self._values[index] = parseret
        return parseret
    ...
    def _parse(self, stream, context, path):
        ...
        for i in range(count):
            try:
                ...
                offset = stream_tell(stream, path)
            offsets[i + 1] = offset
        return LazyListContainer(
            sc, stream, count, offsets, values, context, path
        )
