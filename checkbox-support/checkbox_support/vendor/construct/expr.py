...
class UniExpr(ExprMixin):
    ...
    def __repr__(self):
        return f"{opnames[self.op]} {self.operand!r}"

    def __str__(self):
        return f"{opnames[self.op]} {self.operand}"
...
class BinExpr(ExprMixin):
    ...
    def __repr__(self):
        return f"({self.lhs!r} {opnames[self.op]} {self.rhs!r})"

    def __str__(self):
        return f"({self.lhs} {opnames[self.op]} {self.rhs})"
...
class Path(ExprMixin):
    ...
    def __repr__(self):
        if self.__parent is None:
            ...
            return f"{self.__parent!r}.{self.__field}"

    def __str__(self):
        if self.__parent is None:
            return self.__name
        else:
            return f"{self.__parent}[{self.__field!r}]"
...
class Path2(ExprMixin):
    ...
    def __repr__(self):
        if self.__parent is None:
            ...
            return f"{self.__parent!r}[{self.__index!r}]"
...
class FuncPath(ExprMixin):
    ...
    def __repr__(self):
        if self.__operand is None:
            ...
            return f"{self.__func.__name__}_({self.__operand!r})"

    def __str__(self):
        if self.__operand is None:
            return "%s_" % (self.__func.__name__)
        else:
            return f"{self.__func.__name__}_({self.__operand})"

    def __call__(self, operand, *args):
        if self.__operand is None:
            return (
                FuncPath(self.__func, operand)
                if callable(operand)
                else operand
            )
        else:
