...
def recursion_lock(
    retval="<recursion detected>", lock_name="__recursion_lock__"
):
    """Used internally."""
...
class Container(dict):
    ...
    def __dir__(self):
        ...
        return list(self.keys()) + list(self.__class__.__dict__) + dir(super())
    ...
    def __str__(self):
        ...
        for k, v in self.items():
            ...
            if v.__class__.__name__ == "EnumInteger":
                text.append(f"(enum) (unknown) {v}")
            elif v.__class__.__name__ == "EnumIntegerString":
                text.append(f"(enum) {v} {v.intvalue}")
