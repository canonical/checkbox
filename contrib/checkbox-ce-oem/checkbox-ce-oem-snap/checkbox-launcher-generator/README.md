...
- If none of the three sub-plans are found (or the plan has no
  ...
  with `forced = yes` and `type = interactive` as before.
...
into every generated launcher, in file order.
...
template file. `[ui] type` is the one exception with special handling:
it defaults to `interactive` when the template doesn't set it (a
template may override it too, e.g. to `silent` for unattended runs),
except `type = interactive` always wins whenever `forced = no` is
written (see [Manual / automated / stress
selection](#manual--automated--stress-selection) above), since the
test-plan picker prompt requires it.
