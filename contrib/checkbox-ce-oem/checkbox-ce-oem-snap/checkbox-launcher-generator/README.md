...
it still defaults to `silent`/`interactive` based on whether the
selected plan needs `forced = no` (see [Manual / automated / stress
selection](#manual--automated--stress-selection) above); a template may
override it too, except `type = interactive` always wins whenever
`forced = no` is written, since the test-plan picker prompt requires it.
