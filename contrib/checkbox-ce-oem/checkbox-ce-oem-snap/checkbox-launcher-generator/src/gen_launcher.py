...
# These sections are always generated by gen_launcher.py itself (from the
# selected plan and the editor's manifest/environment items), so a template
# is never allowed to supply them — any such section is silently skipped.
_TEMPLATE_FORBIDDEN_SECTIONS = {
    "launcher",
    "test plan",
    "manifest",
    "environment",
...
def load_launcher_template(
    ...
    """Load extra launcher sections/values from an ini template file.

    This is what lets a user maintain launcher defaults (e.g. ``[ui]``,
    ``[restart]``, ``[report:certification]``, ``[transport:c3]``, ...)
    in one editable ini file instead of hand-patching every generated
    launcher, or forking ``gen_launcher.py``.  ``write_launcher()`` merges
    the result into every launcher it writes.
    ...
    *template_path*, if given, is read directly — this is how a caller
    ...
    Sections named ``launcher``, ``test plan``, ``manifest``, or
    ``environment`` (case-insensitive) are always skipped — those are
    generated by ``gen_launcher.py`` itself and not overridable from a
    template.
    ...
    for section in parser.sections():
        if section.strip().lower() in _TEMPLATE_FORBIDDEN_SECTIONS:
            continue
