...
def check_framebuffer_flips(card: int, samples: int = 10, interval_s: float = 0.2) -> int:
    """
...
def run_flow_nomodeset():
    ...
    if fb0.exists():
        ...
        raise SystemExit("[FAIL] /dev/fb0 missing (expected with nomodeset). Check efifb/simplefb/vesafb/simpledrm.")
    ...
    if fb_sys.is_dir():
        ...
        raise SystemExit("[FAIL] /sys/class/graphics/fb0 not found; fbdev sysfs info missing")
...
def run_flow_kms():
    ...
    if not sys_drm:
        raise SystemExit("[FAIL] /sys/class/drm missing/empty: DRM not exporting state (driver not loaded/bound?)")
    print("[PASS] /sys/class/drm entries: " + ", ".join(p.name for p in sys_drm))

    cards = drm_cards()
    if not cards:
        raise SystemExit("[FAIL] No /sys/class/drm/cardN found: DRM device not registered (driver missing/not bound?)")
    ...
    if not any_driver:
        raise SystemExit("[FAIL] DRM cards exist but none show a bound driver: probe/bind issue")

    # 3) /dev/dri nodes
    dri_nodes = list_dev_dri_nodes()
    if not dri_nodes:
        raise SystemExit("[FAIL] /dev/dri missing/empty: udev/devtmpfs nodes not created")
    ...
    if not has_card:
        raise SystemExit("[FAIL] No /dev/dri/card*: compositor cannot open KMS")
    print("[PASS] /dev/dri/card* present (KMS node)")

    if has_render:
        print("[PASS] /dev/dri/renderD* present (render node)")
    else:
        raise SystemExit("[FAIL] No /dev/dri/renderD*: Mesa may fall back to llvmpipe or rendering may fail")
    ...
    if any(p.startswith("nvidia_drm.modeset=0") for p in params):
        raise SystemExit("[FAIL] nvidia_drm.modeset=0: KMS disabled for NVIDIA DRM (often black screen on Wayland)")
    ...
    for c in cards:
        ...
        for conn in conns:
            ...
            if status == "connected":
                any_connected = True
                if len(modes) == 0:
                    raise SystemExit(f"[FAIL] {ci['name']}: connected but no modes (EDID/AUX/DDC/link issue)")
                if edid_bytes in ("0", "", "?"):
                    print(f"[WARN] {ci['name']}: EDID size suspicious (edid_bytes={edid_bytes})")
                if link_status and link_status.lower() != "good":
                    raise SystemExit(f"[FAIL] {ci['name']}: link_status={link_status}")

    if any_connected:
        print("[PASS] At least one connector is connected")
    else:
        raise SystemExit("[FAIL] No connectors report connected (if you expect display: cable/hotplug/link training)")
