...
"""
This script is copied from the
checkbox/providers/base/bin/sleep_test_log_check.py
...
# Get the list of tests that are running
running_tests_re = re.compile(r"^Running tests: (?P<tests>.+)$")

# These tests don't have the summary section in the log file
# So we need to ignore them
# You can find the list of tests in the `fwts --show-tests`
# And we need to check if all tests are in ignore_summary_tests list
# If all tests are in ignore_summary_tests list, return 0
# If not, return 2
ignore_summary_tests = [
    "acpidump",  # Dump ACPI tables
    "cmosdump",  # Dump CMOS Memory
    "crsdump",  # Dump ACPI _CRS resources
    "dsddump",  # Dump ACPI _DSD (Device Specific Data)
    "ebdadump",  # Dump EBDA region
    "esrtdump",  # Dump ESRT table
    "gpedump",  # Dump GPEs
    "memmapdump",  # Dump system memory map
    "mpdump",  # Dump MultiProcessor Data
    "nhltdump",  # Dump configurations performed via NHLT
    "plddump",  # Dump ACPI _PLD (Physical Device Location)
    "prsdump",  # Dump ACPI _PRS resources
    "romdump",  # Dump ROM data
    "s0idle",  # S0IDLE FADT Low Power S0 Idle tests
...
def main():
    ...
    # levels and tests.
    for logline in log:
        running_tests_matches = running_tests_re.search(logline)
        if running_tests_matches:
            tests = [
                test.strip(".")
                for test in running_tests_matches.group("tests").split()
            ]
            continue
    ...
    if not summaries_found:
        # Check if all tests are in ignore_summary_tests list
        if all(test in ignore_summary_tests for test in tests):
            return_code = 0
        else:
            logging.error(
                "No fwts test summaries found, "
