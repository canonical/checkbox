...
def _validate_packet_data(socket_obj, src_packet, recv_packet):
    ...
    if socket_obj.destruct_packet(recv_packet) != socket_obj.destruct_packet(
        src_packet
    ):
...
def echo_test(interface, can_id, eff_flag, fd_mode):
    ...
    can_id_i, data_b, id_flags = _random_can_data(can_id, eff_flag, data_size)
    ...
    with prepare_can_link(interface, fd_mode):
        ...
        if _validate_packet_data(can_socket, can_pkt, can_recv_pkt):
            logging.info("\nPASSED")
        else:
    ...
    with prepare_can_link(interface, fd_mode):
        ...
        for can_pkt in original_records:
            ...
            if recv_records[-1] is None:
                logging.error("Stop testing due to failed to receive packet")
                logging.error(
...
def register_arguments():
    ...
    server_parser.add_argument("-t", "--time-delay", type=float, default=0.001)
...
def main():
    ...
    if args.mode == "server":
        ...
        if args.execute_test == "stress-echo":
            ...
            echo_test(args.device, args.can_id, args.eff_mode, args.fd_mode)
