...
class CPUScalingTest(object):
    ...
    def initMaximumFrequency(self):
        if not self.setGovernor("performance"):
                return False
        logging.info(
                "Init maximum frequency test time...")
        self.maximumFrequencyTestTime = self.runLoadTest()
        # Verify the current speed is close to scaling_max_freq
        maximumFrequency = self.getParameter("scaling_max_freq")
        currentFrequency = self.getParameter("scaling_cur_freq")
        logging.info(
                "Current cpu frequency is %s MHz" %(int(currentFrequency) / 1000))
        logging.info(
                "Maximum cpu frequecny is %s MHz" %(int(maximumFrequency) / 1000))
        if (
            not maximumFrequency or
            not currentFrequency or
            (float(currentFrequency) < 0.99 * float(maximumFrequency))
