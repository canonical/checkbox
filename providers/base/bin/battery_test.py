    ...
    if not batinfo:
        ...
        if out:
            ...
            if len(batteries) == 0:
                ...
                print(
                    "Warning: This system has more than 1 battery, only the"
...
def battery_life(before, after, time):
    ...
    print(
        "Battery Life with full battery at design capacity (when new): %.2f"
    ...
    print(
        "Battery Life with a full battery at current capacity: %.2f minutes"
        % (current_full_life_minutes)
    )

    # the battery at it's current capacity
    current_life_minutes = round(((before._energy / drain_per_second) / 60), 2)
    print(
        "Battery Life with at current battery capacity: %.2f minutes"
...
def main():
    ...
    parser.add_argument(
        "-i",
        "--idle",
        help="Run the test while system is" " idling",
        action="store_true",
    )
    parser.add_argument(
        "-s3",
        "--sleep",
        help="Run the test while system" " is suspended",
        action="store_true",
    )
    parser.add_argument(
        "-t",
    ...
    parser.add_argument(
        "-m", "--movie", help="Run the test while playing the file MOVIE"
    )
