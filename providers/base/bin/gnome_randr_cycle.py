...
randrinfo = subprocess.Popen("gnome-randr", shell=True, stdout=subprocess.PIPE)
...
for monitor, resolution, mode, rate in highest_modes + current_modes:
    ...
    print(
        "Set mode {}@{} for output {}".format(resolution, rate, monitor),
        flush=True,
    )
    ...
    try:
        ...
        failures = failures + 1
        print(
            "Failed to set mode {} for output {}:".format(mode, monitor),
            file=sys.stderr,
            flush=True,
        )
