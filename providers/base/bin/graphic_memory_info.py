...
def vgamem_paser(data=None):
    ...
    for line in data.split("\n"):
        ...
        for match in re.finditer(r"Memory(.+) prefetchable\) \[size=(\d+)M\]", line):
...
def main():
    ...
    try:
        data = subprocess.check_output(["lspci", "-v"], universal_newlines=True)
    ...
    for vgamem in vgamems:
        ...
        print(
            output_str.format(vgamem["device"], vgamem["name"], vgamem["vgamem_size"])
        )
