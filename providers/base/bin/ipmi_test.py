...
import argparse
import logging
from subprocess import Popen, check_call, PIPE, TimeoutExpired, SubprocessError
    ...
    def __init__(self):
        def get_path(binary):
            ...
            if path_full:
                ...
                logging.info("* Unable to stat absolute path for %s!" % binary)
                return binary
        ...
        self._cmd_ipmi_chassis = [get_path("ipmi-chassis"), "--get-status"]
        self._cmd_ipmi_channel = [
            get_path("ipmi-config"),
            "--checkout",
            "-S",
            "User1",
            "--lan-channel-number",
        ]
        self._cmd_get_bmc_info = [get_path("bmc-info")]
        self._cmd_ipmi_locate = [get_path("ipmi-locate")]
