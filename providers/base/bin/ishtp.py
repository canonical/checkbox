...
def get_release_version():
    try:
        output = subprocess.check_output(["lsb_release", "-rs"], text=True).strip()
        return int(float(output))  # Convert to float first to handle versions like '24.04', then to int
...
def main():
    ...
    parser.add_argument("-m", "--module", action="store_true", help="Check ISHTP kernel modules")
    parser.add_argument("-d", "--device", action="store_true", help="Check ISHTP devices")
