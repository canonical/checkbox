...
def get_pass_through_mac():
    ...
    try:
        ...
        raise SystemExit(
            "No AUXMAC is found in DSDT table, "
...
def check_mac_passthrough(mac, bios_mac):
    ...
    if len(mac) != len(set(mac)):
        print(
            "AUXMAC in DSDT table is identical to onboard NIC MAC, which "
    ...
    if bios_mac in mac:
        print(
            "AUXMAC in DSDT table is passed to dock, MAC address "
...
def check_smbios_token(index):
    ...
    for line in out.stdout.decode("utf-8").splitlines():
        if "value: bool" in line:
            if "true" in line:
                raise SystemExit(
                    "MAC address pass-through " "is disabled in BIOS setting."
                )
            else:
