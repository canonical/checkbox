...
for interface in Path("/sys/class/net").iterdir():
    mod_path = Path(f"{interface}/device/driver/module")
...
for user_driver in sys.argv[1:]:
    if user_driver:
        if Path(f"/sys/module/{user_driver}").exists():
            if not any(x[1] == user_driver for x in driver_list):
                driver_list.append(("unknown", user_driver))
        else:
            print(f"Requested driver {user_driver} not loaded\n")

# Produce the output
for interface, driver in driver_list:
    print(f"Interface {interface} using module {driver}")
    sysfs_path = Path(f"/sys/module/{driver}/parameters")
    if sysfs_path.is_dir():
        ...
        for path in Path(sysfs_path).iterdir():
            if path.is_file():
                ...
                with open(str(path), errors="backslashreplace") as f:
                    print(f"    {path.name}: {f.read().strip()}")
    print()
    print(f"Checking kernel ring buffer for {driver} messages:")
    cmd = f"dmesg -T -x | grep {driver} || true"
