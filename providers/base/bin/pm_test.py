    ...
    def set(cls, minutes=0, seconds=0):
        ...
        with open(cls.ALARM_FILENAME) as alarm_file:
            ...
            if not re.match(r"\d+", wakeup_time_stored_str):
                subprocess.check_call(
                    'echo "+%d" > %s' % (timeout, cls.ALARM_FILENAME),
                    shell=True,
                )
...
class CountdownDialog(Gtk.Dialog):
    ...
    def on_hardware_info_timeout_cb(self):
        ...
        logging.debug(
            ...
            "{xinput}".format(
                xinput=Command("xinput list --name-only | sort").run().stdout
            )
        )
        logging.debug(
            "Pulse Audio Sink:\n"
            "{pactl_sink}".format(
                pactl_sink=(Command("pactl list | grep Sink").run().stdout)
            )
        )
        logging.debug(
            "Pulse Audio Source:\n"
            "{pactl_source}".format(
                pactl_source=(Command("pactl list | grep Source").run().stdout)
            )
        )
...
class MyArgumentParser:
    ...
    def __init__(self):
        ...
        epilog = (
            "Unknown arguments will be passed "
        ...
        parser.add_argument(
            ...
            default=1,
            help=(
                "Number of times that the power management "
        ...
        parser.add_argument(
            "--suspends-before-reboot",
            type=int,
            default=0,
            help=(
                "How many cycles of suspend/resume to run"
        ...
        parser.add_argument(
            "--fwts",
            action="store_true",
            help=("Use fwts " "when doing the suspend tests"),
        )
        parser.add_argument(
            "--checkbox-respawn-cmd",
            type=str,
            help=(
