...
def check_ubuntu_version():
    ...
    if float(version) < 24.04:
        raise ValueError(
            "Ubuntu 24.04 or greater is required, but found {}.".format(version)
        )
...
def check_interface_vendor(interface):
    ...
    if not os.path.exists(vendor_id_path):
        raise FileNotFoundError("Vendor ID path {} not found".format(vendor_id_path))

    with open(vendor_id_path, "r", encoding="utf-8") as file:
        vendor_id = file.read().strip()

    if vendor_id not in VENDOR_INFO:
        raise ValueError("{} has an unknown vendor ID {}".format(interface, vendor_id))
    ...
    try:
        ...
        if not os.path.exists(sriov_path):
            raise FileNotFoundError(
                "SR-IOV not supported or interface {} does not exist.".format(interface)
            )

        logging.info(
            "SR-IOV before change {} VFs on interface {}.".format(num_vfs, interface)
        )
        ...
        logging.info(
            "SR-IOV enabled with {} VFs on interface {}.".format(num_vfs, interface)
        )
...
def main():
    ...
    parser.add_argument("--interface", type=str, default=None, help="SRIOV Interface")
    ...
    lxd_sriov_parser.add_argument("--rootfs", type=str, default=os.getenv("LXD_ROOTFS"))
