...
import dvfs_gpu_test


class DvfsGpuTestCaseBase(unittest.TestCase):
    """Sets up a fake /sys/class/devfreq/<dev>/ tree under a tempdir and
    points dvfs_gpu_test.DEVFREQ_ROOT at it for the duration of the
    test."""
    ...
    def setUp(self):
        ...
        self._orig_root = dvfs_gpu_test.DEVFREQ_ROOT
        dvfs_gpu_test.DEVFREQ_ROOT = self.tmp_path
        self.addCleanup(self._restore_root)

    def _restore_root(self):
        dvfs_gpu_test.DEVFREQ_ROOT = self._orig_root
...
class TestListDevfreqDevices(DvfsGpuTestCaseBase):
    def test_empty_root_returns_empty_list(self):
        self.assertEqual(dvfs_gpu_test.list_devfreq_devices(), [])

    def test_missing_root_returns_empty_list(self):
        dvfs_gpu_test.DEVFREQ_ROOT = self.tmp_path / "does-not-exist"
        self.assertEqual(dvfs_gpu_test.list_devfreq_devices(), [])
...
class TestGovernorsAndFrequencies(DvfsGpuTestCaseBase):
    ...
    def test_get_available_frequencies(self):
        ...
        self.assertEqual(
            dvfs_gpu_test.get_available_frequencies(dev_path),
            [300, 100, 200],
        )

    def test_missing_device_returns_empty(self):
        dev_path = self.tmp_path / "nope"
        self.assertEqual(dvfs_gpu_test.get_available_governors(dev_path), [])
        self.assertEqual(dvfs_gpu_test.get_available_frequencies(dev_path), [])
...
class TestParseDvfsGpuDevices(DvfsGpuTestCaseBase):
    def test_single_device_by_name(self):
        spec = "13000000.gpu|userspace,powersave,performance"
        result = dvfs_gpu_test.parse_dvfs_gpu_devices(spec)
        self.assertEqual(
            result,
            {
                "13000000.gpu": {
    ...
    def test_single_device_no_governors(self):
        result = dvfs_gpu_test.parse_dvfs_gpu_devices("13000000.gpu")
        self.assertEqual(
            result,
            {
                "13000000.gpu": {
    ...
    def test_absolute_path_device(self):
        spec = "/sys/class/misc/mali0/device/devfreq/13000000.gpu"
        result = dvfs_gpu_test.parse_dvfs_gpu_devices(spec)
        self.assertEqual(
            result,
            {
                "13000000.gpu": {
                    "path": Path(
                        "/sys/class/misc/mali0/device/devfreq/" "13000000.gpu"
                    ),
                    "governors": [],
                }
            },
    ...
    def test_multiple_devices(self):
        spec = (
            ...
            "soc:99000000.gpu|userspace,powersave"
        )
        result = dvfs_gpu_test.parse_dvfs_gpu_devices(spec)
        self.assertEqual(
            set(result.keys()), {"13000000.gpu", "soc:99000000.gpu"}
        )
        self.assertEqual(
            result["13000000.gpu"]["governors"],
            ["userspace", "powersave", "performance", "simple_ondemand"],
        )
        self.assertEqual(
            result["soc:99000000.gpu"]["governors"],
            ["userspace", "powersave"],
        )

    def test_invalid_entry_raises(self):
        with self.assertRaises(SystemExit):
            dvfs_gpu_test.parse_dvfs_gpu_devices("|no-name")


class TestResolveDvfsGpuDevices(DvfsGpuTestCaseBase):
    @patch.dict(os.environ, {}, clear=True)
    def test_falls_back_to_default_root_scan(self):
        self.make_device("13000000.gpu")
        self.make_device("soc:vpu_devfreq")
        result = dvfs_gpu_test.resolve_dvfs_gpu_devices()
        self.assertEqual(
            result,
            {
                "13000000.gpu": self.tmp_path / "13000000.gpu",
                "soc:vpu_devfreq": self.tmp_path / "soc:vpu_devfreq",
            },
        )

    @patch.dict(os.environ, {"DVFS_GPU_DEVICES": "13000000.gpu"}, clear=True)
    def test_uses_env_var_when_set(self):
        result = dvfs_gpu_test.resolve_dvfs_gpu_devices()
        self.assertEqual(
            result, {"13000000.gpu": self.tmp_path / "13000000.gpu"}
        )

    def test_uses_absolute_path_from_env_var(self):
        alt_root = self.tmp_path / "alt-root"
        dev_path = self.make_device("13000000.gpu", parent=alt_root)
        with patch.dict(
            os.environ,
            {"DVFS_GPU_DEVICES": "{}".format(dev_path)},
            clear=True,
        ):
            result = dvfs_gpu_test.resolve_dvfs_gpu_devices()
        self.assertEqual(result, {"13000000.gpu": dev_path})


class TestCmdDetect(DvfsGpuTestCaseBase):
    @patch.dict(os.environ, {}, clear=True)
    def test_fails_when_no_devices(self):
        with self.assertRaises(SystemExit):
            dvfs_gpu_test.cmd_detect()
    ...
    @patch.dict(
        os.environ,
        {"DVFS_GPU_DEVICES": "13000000.gpu|userspace,powersave"},
        clear=True,
    )
    ...
    @patch.dict(
        os.environ,
        {"DVFS_GPU_DEVICES": "missing-device|userspace"},
        clear=True,
    )
    ...
    def test_passes_when_using_absolute_path_override(self):
        alt_root = self.tmp_path / "alt-root"
        dev_path = self.make_device("13000000.gpu", parent=alt_root)
        with patch.dict(
            os.environ,
...
class TestCmdResource(DvfsGpuTestCaseBase):
    @patch.dict(os.environ, {}, clear=True)
    def test_returns_none_with_no_devices(self):
        self.assertIsNone(dvfs_gpu_test.cmd_resource())
    ...
    def test_uses_env_var_to_scope_devices(self):
        ...
        with patch.dict(
            os.environ, {"DVFS_GPU_DEVICES": "gpu0"}, clear=True
        ), patch("sys.stdout", new_callable=StringIO) as out:
            dvfs_gpu_test.cmd_resource()
        text = out.getvalue()
        self.assertIn("dvfs_device_name: gpu0", text)
        self.assertNotIn("dvfs_device_name: vpu0", text)


class TestCmdTest(DvfsGpuTestCaseBase):
    ...
    def setUp(self):
        super().setUp()
        self._orig_write_node = dvfs_gpu_test._write_node
        self.addCleanup(self._restore_write_node)
        dvfs_gpu_test._write_node = self._fake_write_node
        patcher = patch.dict(os.environ, {}, clear=True)
        patcher.start()
        self.addCleanup(patcher.stop)

    def _restore_write_node(self):
        dvfs_gpu_test._write_node = self._orig_write_node
    ...
    def _fake_write_node(self, path, value):
        ...
        if path.name == "governor":
            ...
            dev_dir = path.parent
            freqs = dvfs_gpu_test.get_available_frequencies(dev_dir)
    ...
    def test_unknown_device_fails(self):
        with self.assertRaises(SystemExit):
            dvfs_gpu_test.cmd_test("nope", "performance")

    def test_unknown_governor_fails(self):
        self.make_device("dev0")
        with self.assertRaises(SystemExit):
            dvfs_gpu_test.cmd_test("dev0", "bogus")

    def test_performance_locks_to_max_frequency(self):
        self.make_device("dev0", available_frequencies=(100, 200, 300))
        dvfs_gpu_test.cmd_test("dev0", "performance")

    def test_powersave_locks_to_min_frequency(self):
        self.make_device("dev0", available_frequencies=(100, 200, 300))
        dvfs_gpu_test.cmd_test("dev0", "powersave")

    def test_userspace_sweeps_every_frequency(self):
        self.make_device("dev0", available_frequencies=(100, 200, 300))
        dvfs_gpu_test.cmd_test("dev0", "userspace")

    def test_other_governor_only_checks_switch(self):
        self.make_device("dev0")
        dvfs_gpu_test.cmd_test("dev0", "simple_ondemand")

    def test_restores_original_state_after_test(self):
        dev_dir = self.make_device("dev0", governor="userspace", cur_freq=200)
        dvfs_gpu_test.cmd_test("dev0", "performance")
        self.assertEqual(
            dvfs_gpu_test._read_node(dev_dir / "governor"), "userspace"
        )
        self.assertEqual(dvfs_gpu_test._read_node(dev_dir / "cur_freq"), "200")

    def test_fails_when_governor_write_rejected(self):
        self.make_device("dev0")
        with patch.object(dvfs_gpu_test, "_write_node", return_value=False):
            with self.assertRaises(SystemExit):
                dvfs_gpu_test.cmd_test("dev0", "performance")

    def test_resolves_device_via_absolute_path_env_var(self):
        alt_root = self.tmp_path / "alt-root"
        ...
        with patch.dict(
            os.environ,
            {"DVFS_GPU_DEVICES": "{}".format(dev_dir)},
            clear=True,
        ):
            dvfs_gpu_test.cmd_test("dev0", "performance")
        self.assertEqual(
            dvfs_gpu_test._read_node(dev_dir / "governor"),
            "simple_ondemand",
        )
