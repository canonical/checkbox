    ...
    def category(self):
        ...
        if "TYPE" in self._environment and "INTERFACE" in self._environment:
            ...
            if interface_class == Usb.BASE_CLASS_AUDIO:
                # print("Here is a good check point, this audio device is from USB") # DIO
                return "AUDIO"
        ...
        # compatibility.
        if self.bus == "sound":
            # print("Here is another good check point, this audio device is determined by bus") # DIO
            return "AUDIO"
...
class UdevadmParser(object):
    ...
    def _ignoreDevice(self, device):
        ...
        if device.category == "AUDIO":
            print("")
            print("########## device info ########## DIO")
            print("1. ", device)
            print("2. path ", device.path)
            print("3. category ", device.category)
            print("4. bus ", device.bus)
            print("5. product ", device.product)
            print("6. product_id ", device.product_id)
            print("7. vendor ", device.vendor)
            print("8. vendor_id ", device.vendor_id)
            print("9. major ", device.major)
            print("10. _environment ", device._environment)
            print("11. interface ", device.interface)
            print("12. driver ", device.driver)
            print("")
    ...
    def run(self):
        ...
        stack = []
        if isinstance(self.stream_or_string, type("")): # check if the output from "udevadm info -e" is string type
        ...
        for record in re.split("\n{2,}", output):
            ...
            if not record: # to skip the empty part section
                continue
            ...
            for line in record.splitlines():
                ...
                if not line_match: # to skip the wired part section
                    if environment:
                ...
                if key == "P":
                    path = value
                    if "/devices/pci0000:00/0000:00:1f.3/skl_hda_dsp_generic/sound/card0" in path:
            ...
            if not self._ignoreDevice(device):
                if device._raw_path in self.devices:
                    ...
                    self.devices[device._raw_path] = device
                    # print("Here is the 3rd good check point ", device) # DIO
            stack.append(device)
        ...
        for device in list(self.devices.values()):
            if device.category == 'AUDIO':
                print("The last good check point") # DIO
...
def known_to_be_audio_device(vendor_id, product_id, pci_class, pci_subclass):
    # Usually a video device has a PCI subclass_id of Pci.CLASS_DISPLAY_VGA or
    # Pci.CLASS_DISPLAY_3d. However, some manufacturers which hadn't previously
    # used a subclass_id of Pci.CLASS_DISPLAY_OTHER (0x80 or decimal 128) have
    # begun to do so (as of 2013-2014); and others which previously used this
